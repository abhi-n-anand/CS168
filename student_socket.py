...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    pack = self.new_packet(False, None, True)
    pack.tcp.seq = self.snd.iss
    self.state = SYN_SENT
    self.tx(pack)
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    # Complete for Stage 3 (check recv queue)
      ...
      elif self.rx_queue.peek()[0] |GT| self.rcv.nxt:
        self.set_pending_ack()
      else:
        while not self.rx_queue.empty() and self.rx_queue.peek()[0] |LE| self.rcv.nxt:
          seq, packet = self.rx_queue.pop()
          seg = packet.tcp
          payload = seg.payload[self.rcv.nxt |MINUS| seg.seq:]
  ...
  def handle_synsent (self, seg):
    ...
    if acceptable_ack:
      ...
      if self.snd.una |GT| self.snd.iss: #GT = greater than; Stage 1, Number 4 --> if the new oldest unacknowledged sequence number is larger than our initial sequence number
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.srtt == 0 and self.rttvar == 0 and self.rto == 1:
      self.srtt = rtt
      self.rttvar = rtt / 2
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    if (self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2)) and (len(payload) |GT| 0):
  ...
  def maybe_send(self):
    ...
    dataInFlight = snd.nxt |MINUS| snd.una
    bytes_sent = dataInFlight

    segLen = self.mss
    remaining = len(self.tx_data)

    while remaining > 0 and bytes_sent |LE| windowSize:
      segLen = min(remaining, self.mss, windowSize - bytes_sent)
      payload = self.tx_data[:segLen]

      newPack = self.new_packet(data=payload)
      self.tx(newPack)
      self.tx_data = self.tx_data[segLen:]

      num_pkts += 1
      bytes_sent += len(newPack)
  ...
  def check_timer_retx(self):
    ...
    if not self.retx_queue.empty():
      tup = self.retx_queue.get_earliest_pkt() #Returns (does not remove) the tuple (seq_no, p) where p is the packet with the minimum transmission timestamp (tx_ts), and seq_no is p's sequence number.
      seq = tup[0]
      p = tup[1]
